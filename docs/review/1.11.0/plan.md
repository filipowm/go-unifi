...
|------------------------|-------------------|---------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| TLS default            | ARCH-06           | **Default to verify-ON, warn when disabled.** Shipped via rename + inversion (a deliberate, documented hard API break). | A plain `bool` zero-value is `false`, so "default verify-ON" is **not expressible while keeping the positive `VerifySSL` field**. Three options were on the table: (a) `VerifySSL *bool` (`nil` ⇒ verify), (b) a functional-option, (c) **invert to a secure-by-default plain bool**. Shipped (c): rename `ClientConfig.VerifySSL bool` → **`SkipVerifySSL bool`** (zero value `false` ⇒ verify; `true` ⇒ skip) — matches the architect's original recommendation, keeps plain-bool ergonomics (no `*bool`/`new(false)` ceremony), and the rename forces a compile error at every call site so the secure-by-default flip can't slip through silently. Emit a `Warn` log on every client build where verification ends up disabled. Document prominently as a breaking change.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| ARCH-06 | TLS verify-by-default + warn via `VerifySSL bool` → `SkipVerifySSL bool` rename/inversion (see decision table). Update README/config docs.                                                                                                | M      | Med  | —             |
