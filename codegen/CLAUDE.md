...
oapi-codegen's allOf+discriminator path drops variant structs, so `transform.go`/`naming.go` rewrite the spec into a oneOf union form — deterministic, fail-loud: downconvert 3.1→3.0.3, synthesize a `oneOf` per discriminator, diamond-fix (variant extending 2+ parents), enum dedup, collision-rename. Shape classifier (`resources.go`+`surface.go`) maps ops by `operationId`+method+params: list GETs (a `data[]` page envelope)→`[]…Overview`, single GETs→`*…Details`, `Create/Update/Patch*`→`…CreateOrUpdate`. **A list GET emits TWO wrappers** (`methodsFor`, keyed by `methodKind`) so draining is explicit, never the default: `List<Qualifier>Page(ctx, …, *ListOptions) (Page[T], error)` fetches one **bounded** page (nil opts ⇒ first page, `Limit` clamped to `maxPageLimit`=200), and `List<Qualifier>All(ctx, …) iter.Seq2[T, error]` is a lazy, abortable drain (Go 1.23+ range-over-func) — runs the gate before paging, forwards the filter on every request, stops on an empty page / `offset==totalCount` / consumer `break`. `ListOptions`, `Page[T]`, `Collect`, `listPage`/`listSeq` are hand-written in `unifi/official/pagination.go` (offset/limit/filter are the standardized list-envelope params, owned there — except a REQUIRED filter on a non-list op like bulk `deleteVouchers`, which stays an explicit string arg).

**Fluent, per-group surface** (`groups.go`+`surface.go`) — the surface is grouped by OpenAPI tag, not flat: each operation's primary tag selects a group (`operationGroup`); docs-only/zero-op tags auto-skip. The generator emits one `<group>.generated.go` per group (the `<Group>Client` interface, its `*apiClient` accessor + impl, the wrapper impls, and a per-group func-field `<Group>ClientMock`) plus a parent `client.generated.go` (the `Client` interface with one accessor per group). `groupName` PascalCases the tag with `groupOverrides` tidying ambiguous ones; convention is **plural for true resource collections** (`DNSPolicies`, `ACLs`, `TrafficMatchingLists`), **singular for feature areas** (`Firewall`, `Hotspot`, `Supporting`, `Info`) — the go-github/k8s/Stripe idiom; an unlisted new tag auto-yields a new group (caught by the golden diff). `methodName` strips the group's resource word(s) so methods read cleanly under their accessor (`createFirewallPolicy` under `Firewall()`→`CreatePolicy`), then `normalizeReadVerb` makes reads uniform: a collection read becomes `List<Qualifier>` (`Get`→`List`, the `Page`/`Overview`/`List(s)` envelope words dropped — `getNetworksOverviewPage`→`List`, `getFirewallPolicies`→`ListPolicies`) and a single-item GET drops a trailing `Details` (`getNetworkDetails`→`Get`). `stemOverrides` supplies the singular token set for pluralised groups (`DNSPolicies` strips `["DNS","Policy"]` so `createDnsPolicy`→`Create`); a post-strip collision fails loud (`buildGroups`/`assertNoCollision`). The hand-written methods are **re-homed onto groups** (`Info().Get`, `Sites().ListPage`/`Sites().ListAll`, `Sites().ResolveID`): the generator emits their interface/mock/accessor but the body lives in `unifi/official/info.go`/`sites.go` on the `infoClient`/`sitesClient` impls, so a group mixes generated wrappers with preserved hand-written members.

**Validation tags** (`validation.go`) — `injectValidationTags` stamps a go-playground `validate` struct-tag (via oapi-codegen's `x-oapi-codegen-extra-tags`) onto every constrained property, derived from the spec. It runs inside `Transform` **before** `dedupeEnums` so inline enum values are still readable; `enumRef` then carries the `x-*` tag through dedup. Constraint→rule map:

| OpenAPI | go-playground | Applies to |
|---|---|---|
| `enum` | `oneof=v1 v2 …` (scalar) / `dive,oneof=…` (array) | any |
| `minimum` / `maximum` | `gte=` / `lte=` (inclusive) | numeric |
| `minLength` / `maxLength` | `min=` / `max=` | string |
| `minItems` / `maxItems` | `min=` / `max=` | slice |

Every rule **leads with `omitempty`** (validate-level, pointer-safe): JSON omitempty ≠ validator omitempty, so a bare `gte=1` on a nil `*int32` false-fails. `required` is **not** emitted (OpenAPI key-present ≠ go-playground non-zero; oapi-codegen renders required fields as bare value types, so it would false-fail a legit-false bool / zero int) and neither is `format` (uuid→`UUID` type, date-time→`time.Time`). Quirks: the `maxItems: 2147483647` (math.MaxInt32) sentinel is dropped; integer-typed properties with string enum values pass through verbatim; the injector **fails loud** on any enum value containing a space, comma, or pipe (space splits into bogus `oneof` members; comma corrupts the whole validate tag at the go-playground level; pipe is its OR-separator). **`omitempty,min=N` where N ≤ 1 is a no-op**: any value that survives the `omitempty` zero-value check trivially satisfies `min=1`, so `lengthRules` only emits `min=N` when N > 1 (the genuine `min=8` passphrase case); `min=0` is always-true and is equally suppressed. **`oneof` is not emitted for `type: "number"` (float) fields**: go-playground's `isOneOf` only supports string/int/uint and panics on float32/float64; float enum fields carry no `oneof` rule (`.Valid()` on the generated type covers enum membership). Official request bodies are validated under the parent client's `ValidationMode` — default `SoftValidation` only warns, opt-in `HardValidation` blocks — so a newly-emitted tag is **not breaking by default**.
