...
**Resource floor (two-snapshot merge).** `unifi/codegen.go` also passes `-floor-version=9.0.114`. The Internal pass then generates the resource set as a merge of two snapshots (`internal.buildMergedResources` → `mergeResourceSets`): the **9.0.114 floor** bounds the resource membership below, the **9.5.21** snapshot supplies the newest field shapes (newest wins at struct-name granularity — whole-resource replacement, not field-level union). Resources retired **before** 9.0.114 are absent from both snapshots and never generated; resources added after the floor (present only in 9.5.21) are kept. At resource level 9.0.114 ⊂ 9.5.21 (74 vs 77 resources), so no resource type is dropped — but the merge is **not a no-op at field level**: 17 fields present only in the 9.0.114 snapshot across `ChannelPlan`, `SettingIps`, `SettingSuperMgmt`, and `NetworkConf` are intentionally dropped because 9.5.21's struct wins in full (see `docs/2.0.0/breaking_changes.md` §G for the full list). The floor's value is structural: it bounds resource membership so the floor can never be silently exceeded. An empty `-floor-version` disables the merge (single-snapshot generation, the path used by unit tests).
