...
an absolute (leading-slash) path. Unlike legacy REST resources, the Official surface accepts `PATCH` for partial
updates:
...
// Absolute path under the Official integration surface accepts PATCH.
path := "/proxy/network/integration/v1/sites/<site-id>/networks/<network-id>"
...
<Callout type="warn">
`Patch` sends a literal HTTP `PATCH`. The Official `integration/v1` surface accepts it, but most legacy Internal
REST resources (e.g. `networkconf`) expect a full `PUT` — use `Put` for those, sending the complete object.
</Callout>
