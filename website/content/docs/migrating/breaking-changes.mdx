...
`// Deprecated:` and has **no effect** — setting it `true` or `false` changes nothing. This no-op is **not new
in 2.0.0**: `UseLocking` has been deprecated and inert since **1.11.0**, so 1.11.x callers already saw no
serialisation. Remove it from your config.
...
- `ErrOfficialAPIUnavailable` on an old-style (classic) controller, a failed `GET /v1/info` probe (a rejected
  API key surfaces here), or a controller version below 10.1.78.
