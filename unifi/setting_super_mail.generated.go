...
func init() { //nolint:gochecknoinits
	registerSetting(SettingSuperMailKey, func() any { return &SettingSuperMail{} })
}
...
func (c *client) GetSettingSuperMail(ctx context.Context, site string) (*SettingSuperMail, error) {
	...
	resource, ok := f.(*SettingSuperMail)
	if !ok {
		return nil, fmt.Errorf("unexpected type for setting value. expected: *SettingSuperMail, received: %T", f)
	}
	return resource, nil
}
...
func (c *client) UpdateSettingSuperMail(ctx context.Context, site string, s *SettingSuperMail) (*SettingSuperMail, error) {
	...
	updatedResource, ok := result.(*SettingSuperMail)
	if !ok {
		return nil, fmt.Errorf("unexpected type for setting value. expected: *SettingSuperMail, received: %T", result)
	}
