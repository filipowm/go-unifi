...
type FirewallRule struct {
	...
	ProtocolV6            string   `json:"protocol_v6,omitempty"`                                                                                                                                                                                                            // ^$|([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])|ah|all|dccp|eigrp|esp|gre|icmpv6|ipcomp|ipv6|ipv6-frag|ipv6-icmp|ipv6-nonxt|ipv6-opts|ipv6-route|isis|l2tp|manet|mobility-header|mpls-in-ip|ospf|pim|rsvp|sctp|shim6|tcp|tcp_udp|udp|vrrp
	...
	SrcMACAddress         string   `json:"src_mac_address,omitempty" validate:"omitempty,mac"`                     // ^([0-9A-Fa-f]{2}:){5}([0-9A-Fa-f]{2})$|^$
	SrcNetworkID          string   `json:"src_networkconf_id,omitempty" validate:"omitempty,w_regex"`              // [\d\w]+|^$
...
func (c *client) createFirewallRule(ctx context.Context, site string, d *FirewallRule) (*FirewallRule, error) {
	...
	if len(respBody.Data) != 1 {
		return nil, fmt.Errorf("unexpected response: expected 1 FirewallRule, got %d", len(respBody.Data))
	}
