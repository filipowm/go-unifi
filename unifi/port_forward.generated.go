...
type PortForward struct {
	...
	PfwdInterface      string                      `json:"pfwd_interface,omitempty"`                                   // wan[2-9]?|both|all
...
func (c *client) createPortForward(ctx context.Context, site string, d *PortForward) (*PortForward, error) {
	...
	if len(respBody.Data) != 1 {
		return nil, fmt.Errorf("unexpected response: expected 1 PortForward, got %d", len(respBody.Data))
	}
